...
#ifdef PATCHED
    if (receive_delim(0, one.name, sizeof(one.name), DELIM_CHAR, &(one.namelen)) != 0) {
        _terminate(0);
    }
    one.namelen--;
#else
    if (receive_delim(0, one.name, 64, DELIM_CHAR, &rxlen) != 0) {
        _terminate(0);
    }      
#endif
...
#ifdef PATCHED
    if (receive_delim(0, two.name, sizeof(two.name), DELIM_CHAR, &(two.namelen)) != 0) {
        _terminate(0);
    }
    two.namelen--;
#else
    if (receive_delim(0, two.name, 64, DELIM_CHAR, &rxlen) != 0) {
        _terminate(0);
    }
#endif
...
int turn(player *current){
    ...
    print(*current, TURN);
#ifdef PATCHED
    if (receive_delim(0, input, sizeof(input), DELIM_CHAR, &rxlen) != 0) {
        _terminate(0);
    }
#else
    if (receive_delim(0, input, 32, DELIM_CHAR, &rxlen) != 0) {
        _terminate(0);
    }
#endif
...
int lenofname(char *name){
    int i;
    int namelen = 0;
    for(i = 0; name[i] != '\0' && name[i] != DELIM_CHAR; i++){
        namelen++;
    } 
    return namelen;
}
