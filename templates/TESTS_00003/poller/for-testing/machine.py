    ...
    def register(self):
        ...
        for id in self.state['id_subset']:
            ...
            if  (self.state['employees'][id].exempt):
                self.write(EMPLOYEE_ID_STR+"-{a.id}`employee_name-{a.name}`wage-{a.wagedollars}.{a.wagecents:02}`exempt-yes`\n".format(a=self.state['employees'][id]))
            else:
                self.write(EMPLOYEE_ID_STR+"-{a.id}`employee_name-{a.name}`wage-{a.wagedollars}.{a.wagecents:02}`\n".format(a=self.state['employees'][id]))
    ...
    def loghours(self):
        for id in self.state['id_subset']:
            for week in self.state['weeks_subset']:
                self.write(EMPLOYEE_ID_STR+"-{}`week-{}`".format(id, week))
                if self.chance(0.9):    
            ...
            if self.chance(0.33):
                for week in range(0,52):
                    if self.chance(0.05):
                        ...
                        self.write("query-week`"+EMPLOYEE_ID_STR+"-{}`week-{}`\n".format(id, week))
                        self.read(delim='\n', expect=self.get_response(id, week))
            elif self.chance(0.50):
                self.write("query-one`"+EMPLOYEE_ID_STR+"-{}`\n".format(id))
                response = " "
                for week in range(0,52):
                   self.read(delim='\n', expect=self.get_response(id, week))
            else:
