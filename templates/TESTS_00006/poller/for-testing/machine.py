...
class Pttp(Actions):
    ...
    def auth(self):
        if 'has_token' in self.state:
            self.write(TOKEN_AUTH + ' ', self.state['has_token'], '\n')
            self.read(delim='\n', expect='OK\n')
        else:
            self.write(TOKEN_AUTH + ' ' + random_string(random.randint(1, 10)) + '\n')
            self.read(delim='\n')
    ...
    def call_random(self):
        ...
        self.write(TOKEN_CALL + ' ' + s + '\n')
        self.read(delim='\n', expect='ERROR 404\n')

    def call_home(self):
        self.write(TOKEN_CALL + ' /\n')
    ...
    def set_random(self):
        ...
        self.write(TOKEN_SET + ' {0} {1}\n'.format(arg, val))
        self.read(delim='\n', expect="OK\n")

    def set_mode(self):
        if random.randint(0,1):
            self.write(TOKEN_SET + ' mode encode\n')
            self.state['mode'] = 'encode'
        else:
            self.write(TOKEN_SET + ' mode decode\n')
            self.state['mode'] = 'decode'
        self.read(delim='\n', expect="OK\n")

    def set_data(self):
        data = random_string(random.randint(1, 127))
        self.write(TOKEN_SET + ' data {0}\n'.format(data))
        self.read(delim='\n', expect="OK\n")
        self.state['data'] = data

    def set_key(self):
        key = random_string(random.randint(1, 127))
        self.write(TOKEN_SET + ' key {0}\n'.format(key))
        self.read(delim='\n', expect="OK\n")
        self.state['key'] = key

    def bye(self):
        self.write(TOKEN_BYE + '\n')
        self.read(delim='\n', expect='BYE\n')
