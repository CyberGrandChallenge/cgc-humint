...
def encodeHexStr(s) :
    ...
    for _ in s :
      if (_ >= '0') and (_ <= '9') :
        ret += chr(ord(PY_ZERO_CHAR) + (ord(_) - ord('0')))
      else :
        ret += _
    return ret
...
class Benchmark(Actions):
    ...
    def start(self):
        ...
        output = ','.join([encodeHexStr(self.funcNum), encodeHexStr(self.count), encodeHexStr(self.printGran), encodeHexStr(self.option1), encodeHexStr(self.option2), encodeHexStr(self.bStopOnError)])
